...
type FieldNamer interface {
	GetJSONFieldName(t *types.Struct, fieldOffset int) string
}
...
func NewGenerator(filename string) *Generator {
	ret := &Generator{
		...
		marshalers:    make(map[types.Type]bool),
		typesSeen:     make(map[types.Type]bool),
...
func (g *Generator) addType(t types.Type) {
	if g.typesSeen[t] {
		return
	}
...
func (g *Generator) Add(t types.Type) {
	g.addType(t)
	g.marshalers[t] = true
}

// printHeader prints package declaration and imports.
func (g *Generator) printHeader(w io.Writer) error {
	b := &bytes.Buffer{}

	if g.buildTags != "" {
		fmt.Fprintln(b, "// +build ", g.buildTags)
		fmt.Fprintln(b)
	}
	fmt.Fprintln(b, "// Code generated by easyjson for marshaling/unmarshaling. DO NOT EDIT.")
	fmt.Fprintln(b)
	...
	fmt.Fprintln(b, ")")
	fmt.Fprintln(b, "")
	fmt.Fprintln(b, "// suppress unused package warning")
	fmt.Fprintln(b, "var (")
	fmt.Fprintln(b, "   _ *json.RawMessage")
	fmt.Fprintln(b, "   _ *jlexer.Lexer")
	fmt.Fprintln(b, "   _ *jwriter.Writer")
	fmt.Fprintln(b, "   _ easyjson.Marshaler")
	fmt.Fprintln(b, ")")
...
func (g *Generator) Run(out io.Writer) error {
	...
	if err := g.printHeader(out); err != nil {
		return err
	}
...
func (g *Generator) getType(t types.Type) string {
	named, isNamed := t.(*types.Named)
	if !isNamed {
		switch v := t.(type) {
		case *types.Pointer:
			return "*" + g.getType(v.Elem())
		case *types.Slice:
			return "[]" + g.getType(v.Elem())
		case *types.Array:
			return "[" + strconv.Itoa(int(v.Len())) + "]" + g.getType(v.Elem())
		case *types.Map:
			return "map[" + g.getType(v.Key()) + "]" + g.getType(v.Elem())
		}
	}

	if !isNamed || named.Obj().Name() == "" || named.Obj().Pkg() == nil {
		if v, ok := t.Underlying().(*types.Struct); ok {
			// the fields of an anonymous struct can have named types,
			// and v.String() will not be sufficient because it does not
			...
			for i := 0; i < nf; i++ {
				...
				t := v.Tag(i)
				if t != "" {
					line += " " + escapeTag(reflect.StructTag(t))
				}
		...
		return t.String()
	} else if named.Obj().Pkg().Path() == g.pkgPath {
		return named.Obj().Name()
	}
	return g.pkgAlias(named.Obj().Pkg().Path()) + "." + named.Obj().Name()
}
...
func (g *Generator) safeName(t types.Type) string {
	n := t.(*types.Named)
	name := n.Obj().Pkg().Path()
	if n.Obj().Name() == "" {
		name += "anonymous"
...
func (g *Generator) functionName(prefix string, t types.Type) string {
...
func (DefaultFieldNamer) GetJSONFieldName(t *types.Struct, i int) string {
...
func (LowerCamelCaseFieldNamer) GetJSONFieldName(t *types.Struct, i int) string {
	tag := reflect.StructTag(t.Tag(i))
	jsonName := strings.Split(tag.Get("json"), ",")[0]
	if jsonName != "" {
		return jsonName
...
func (SnakeCaseFieldNamer) GetJSONFieldName(s *types.Struct, i int) string {
	...
	return camelToSnake(s.Field(i).Name())
}
